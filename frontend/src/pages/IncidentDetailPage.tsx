...
export function IncidentDetailPage() {
  ...
  useEffect(() => { load() }, [load])
  useEffect(() => subscribeToIncidentEvents(event => { if (event.incidentId === id) load() }), [id, load])
  ...
  if (error) return <main className="workspace"><h1>Incident unavailable</h1><p>{error} Check that Pulse is running, then try again.</p><button onClick={load}>Try again</button><p><Link className="back-link" to="/incidents">← Return to incidents</Link></p></main>
